...
`current_offer_fingerprints` stores one stable hash per `source_product_id` over current price, retail/promo price, unit price, sale/promo flags, availability, canonical id, chain/store/locality, and source-file provenance. `offer_change_events` is the future append-only event stream. `snapshot_manifests` records scanned counts, diff counts, affected canonical ids, category samples, and estimated writes.

Diff categories are `unchanged`, `new_offers`, `price_changed`, `promo_changed`, `availability_changed`, `metadata_changed_only`, `canonical_mapping_changed`, `other_changed`, and `missing_removed`. Dry-runs expose estimated write variants for `all_changes`, `price_promo_availability`, and `none`; the default recommendation is `price_promo_availability` so metadata-only changes update current state without creating audit events. Missing/removed offers are reported but not deleted by default.

Dry-runs also print `diff_diagnostics` for pre-writer audits: top chains/retailers and categories for new/missing/price/mapping changes, sample source products, price old/new samples, canonical mapping before/after samples, source-id shape checks, and a replacement heuristic that estimates likely same-real-offer churn when both missing baseline rows and new snapshot rows contain normalized name, chain/store, and category fields. Compact baselines may not contain enough old-side detail, and the diagnostic must report that limitation explicitly.
