...
async function main() {
  ...
  if (!isPostgresConfigured(process.env)) {
    console.log('Postgres is not configured. Set DATABASE_URL or POSTGRES_* before reviewing recipe nutrition profiles.');
    return;
  }
...
async function runCommand(client, args) {
  if (args.listApproved) {
    return {
      ...
      profiles: await listApprovedRecipeNutritionProfiles(client, args),
    };
  }
  if (args.candidateId && !args.decision) {
    return {
      action: 'show_candidate',
      detail: await getRecipeNutritionProfileCandidateDetail(client, { candidateId: args.candidateId }),
    };
  }
  if (args.decision) {
    return {
      action: 'review_candidate',
      result: await reviewRecipeNutritionProfileCandidate(client, args),
    };
  }
  return {
    action: 'list_candidates',
    candidates: await listRecipeNutritionProfileCandidatesForReview(client, args),
  };
}
...
function human(report) {
  if (report.action === 'list_approved_profiles') return `Approved recipe profiles: ${report.profiles.length}`;
  if (report.action === 'show_candidate') {
    return report.detail
      ? `${report.detail.candidate.recipe_profile_candidate_id}: ${report.detail.candidate.review_status}`
      : 'Candidate not found.';
  }
  if (report.action === 'review_candidate') {
    return `${report.result.candidate.recipe_profile_candidate_id}: ${report.result.previous_candidate_review_status} -> ${report.result.review_decision}`;
  }
  return `Recipe profile candidates: ${report.candidates.length}`;
}
